import React, { useState, useEffect, useMemo, useRef } from 'react';
import { Check, X, ArrowRight, RotateCcw, Volume2, Sparkles, HelpCircle } from 'lucide-react';
import { UserProfile, VocabPracticeResult, UnitContextQuestion } from '../../types';
import { VOCABULARY_RAW_DATA } from '../../data/vocabularyData';
import { store } from '../../services/store';
import { audioService } from '../../services/audioService';
import {
  getUnitContextQuestions,
  checkWordAnswerMatch,
} from '../../data/vocabularyPracticeData';
import { PracticeResultSummary } from './PracticeResultSummary';

interface CompleteSentenceExerciseProps {
  unit: number;
  user: UserProfile;
  onComplete: () => void;
  onNext: () => void;
  onBackToRoadmap: () => void;
}

export const CompleteSentenceExercise: React.FC<CompleteSentenceExerciseProps> = ({
  unit,
  user,
  onComplete,
  onNext,
  onBackToRoadmap,
}) => {
  const contextQuestions = useMemo(() => {
    return getUnitContextQuestions(unit);
  }, [unit]);

  const unitVocab = useMemo(() => {
    const raw = VOCABULARY_RAW_DATA.find((u) => u.unit === unit);
    return raw ? raw.vocabulary : [];
  }, [unit]);

  const [questions, setQuestions] = useState<UnitContextQuestion[]>([]);
  const [currentIndex, setCurrentIndex] = useState(0);
  const [typedAnswer, setTypedAnswer] = useState('');
  const [isAnswerSubmitted, setIsAnswerSubmitted] = useState(false);
  const [isCorrect, setIsCorrect] = useState(false);
  const [firstAttemptMap, setFirstAttemptMap] = useState<Record<number, boolean>>({});
  const [retryMap, setRetryMap] = useState<Record<number, boolean>>({});
  const [score, setScore] = useState(0);
  const [isFinished, setIsFinished] = useState(false);
  const [attemptCount, setAttemptCount] = useState(1);
  const [resultData, setResultData] = useState<VocabPracticeResult | null>(null);

  const inputRef = useRef<HTMLInputElement>(null);

  const initExercise = () => {
    if (contextQuestions.length === 0) return;
    setQuestions([...contextQuestions]);
    setCurrentIndex(0);
    setTypedAnswer('');
    setIsAnswerSubmitted(false);
    setIsCorrect(false);
    setFirstAttemptMap({});
    setRetryMap({});
    setScore(0);
    setIsFinished(false);
    setResultData(null);
  };

  useEffect(() => {
    initExercise();
  }, [unit]);

  const currentQ = questions[currentIndex];

  // Matching vocab item for meaning / audio
  const matchedVocabItem = useMemo(() => {
    if (!currentQ) return null;
    return (
      unitVocab.find((v) => checkWordAnswerMatch(v.word, currentQ.answer)) ||
      unitVocab.find((v) => v.word.toLowerCase().includes(currentQ.answer.toLowerCase()))
    );
  }, [currentQ, unitVocab]);

  // Suggested word bank options (target + 3 distractors)
  const wordBankOptions = useMemo(() => {
    if (!currentQ) return [];
    const otherAnswers = contextQuestions
      .filter((q) => q.id !== currentQ.id)
      .map((q) => q.answer)
      .sort(() => 0.5 - Math.random())
      .slice(0, 3);

    return [currentQ.answer, ...otherAnswers].sort(() => 0.5 - Math.random());
  }, [currentQ, contextQuestions]);

  useEffect(() => {
    if (inputRef.current && !isAnswerSubmitted && !isFinished) {
      inputRef.current.focus();
    }
  }, [currentIndex, isAnswerSubmitted, isFinished]);

  const handleSubmitAnswer = (e?: React.FormEvent) => {
    if (e) e.preventDefault();
    if (!typedAnswer.trim() || isAnswerSubmitted) return;

    const correct = checkWordAnswerMatch(typedAnswer, currentQ.answer);
    setIsCorrect(correct);
    setIsAnswerSubmitted(true);

    const isFirst = firstAttemptMap[currentIndex] === undefined && !retryMap[currentIndex];

    if (correct) {
      if (isFirst) {
        setFirstAttemptMap((prev) => ({ ...prev, [currentIndex]: true }));
        setScore((prev) => prev + 10);
      } else {
        setScore((prev) => prev + 5);
      }

      if (matchedVocabItem) {
        store.recordWordPractice(
          user.id,
          unit,
          matchedVocabItem.id,
          matchedVocabItem.word,
          matchedVocabItem.meaning_vi,
          matchedVocabItem.ipa,
          true,
          isFirst
        );
      }
    } else {
      setFirstAttemptMap((prev) => ({ ...prev, [currentIndex]: false }));
      if (matchedVocabItem) {
        store.recordWordPractice(
          user.id,
          unit,
          matchedVocabItem.id,
          matchedVocabItem.word,
          matchedVocabItem.meaning_vi,
          matchedVocabItem.ipa,
          false,
          false
        );
      }
    }
  };

  const handleSelectWordChip = (word: string) => {
    if (isAnswerSubmitted) return;
    setTypedAnswer(word);
  };

  const handleRetryCurrent = () => {
    setRetryMap((prev) => ({ ...prev, [currentIndex]: true }));
    setTypedAnswer('');
    setIsAnswerSubmitted(false);
  };

  const handleNextQuestion = () => {
    if (currentIndex < questions.length - 1) {
      setCurrentIndex((prev) => prev + 1);
      setTypedAnswer('');
      setIsAnswerSubmitted(false);
      setIsCorrect(false);
    } else {
      const maxScore = questions.length * 10;
      const percentage = Math.round((score / maxScore) * 100);
      const passed = percentage >= 80;

      const result: VocabPracticeResult = {
        unit,
        studentId: user.id,
        practiceType: 'complete_sentence',
        score,
        maxScore,
        percentage,
        passed,
        attemptNumber: attemptCount,
        bestScore: percentage,
        latestScore: percentage,
        correctCount: Object.values(firstAttemptMap).filter(Boolean).length,
        totalQuestions: questions.length,
        wordsPracticed: questions.map((q, idx) => ({
          word: q.answer,
          isCorrect: Boolean(firstAttemptMap[idx]),
        })),
        createdAt: new Date().toISOString(),
      };

      store.saveVocabPracticeResult(result);
      setResultData(result);
      setIsFinished(true);
      onComplete();
    }
  };

  const handleRetryAll = () => {
    setAttemptCount((prev) => prev + 1);
    initExercise();
  };

  if (isFinished && resultData) {
    const wrongList = questions
      .filter((_, idx) => !firstAttemptMap[idx])
      .map((q) => {
        const item = unitVocab.find((v) => checkWordAnswerMatch(v.word, q.answer));
        return {
          word: q.answer,
          meaningVi: item?.meaning_vi || 'Xem lại nghĩa từ trong ngữ cảnh',
          ipa: item?.ipa,
        };
      });

    return (
      <PracticeResultSummary
        practiceType="complete_sentence"
        score={resultData.score}
        maxScore={resultData.maxScore}
        percentage={resultData.percentage}
        passed={resultData.passed}
        attemptNumber={attemptCount}
        wrongWords={wrongList}
        onRetry={handleRetryAll}
        onNext={onNext}
        onBackToRoadmap={onBackToRoadmap}
      />
    );
  }

  if (!currentQ) return null;

  // Split sentence around blank
  const parts = currentQ.sentence.split('______');
  const fullSentenceWithAnswer = currentQ.sentence.replace('______', currentQ.answer);

  return (
    <div className="max-w-2xl mx-auto space-y-6 pt-2 pb-12 animate-fadeIn">
      {/* Header Info */}
      <div className="bg-white rounded-3xl p-5 sm:p-6 border border-[#E5DDD0] shadow-xs flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="space-y-1">
          <div className="flex items-center gap-2">
            <span className="text-xl">📝</span>
            <span className="text-xs font-black uppercase tracking-wider text-[#4B5D44] bg-[#E8EFE6] px-2.5 py-0.5 rounded-full border border-[#C6D8C2]">
              Luyện từ vựng • Bài 5/6
            </span>
          </div>
          <h2 className="text-xl sm:text-2xl font-black text-[#2D332A] font-serif">
            Điền từ vào câu ngữ cảnh
          </h2>
          <p className="text-xs sm:text-sm text-[#5C6B57]">
            Điền từ hoặc cụm từ vựng thích hợp vào chỗ trống trong câu.
          </p>
        </div>

        {/* Question Counter */}
        <div className="flex sm:flex-col items-center sm:items-end justify-between sm:justify-center border-t sm:border-t-0 pt-2 sm:pt-0 border-[#E5DDD0]">
          <span className="text-xs text-[#5C6B57] font-bold">Câu hỏi</span>
          <span className="text-lg font-black text-[#2D332A] font-serif">
            {currentIndex + 1} <span className="text-xs text-[#8C9886]">/ {questions.length}</span>
          </span>
        </div>
      </div>

      {/* Sentence Card */}
      <div className="bg-white rounded-3xl p-6 sm:p-8 border border-[#E5DDD0] shadow-sm text-center space-y-5">
        <div className="text-lg sm:text-xl font-bold text-[#2D332A] leading-relaxed">
          <span>{parts[0]}</span>
          <span
            className={`inline-block px-3 py-1 mx-1.5 rounded-xl border font-black ${
              isAnswerSubmitted
                ? isCorrect
                  ? 'bg-[#E8EFE6] border-[#4B5D44] text-[#384732]'
                  : 'bg-[#F9EBE9] border-[#E8C2BD] text-[#88372A]'
                : typedAnswer
                ? 'bg-[#FAF7F2] border-[#4B5D44] text-[#2D332A]'
                : 'bg-[#FAF7F2] border-dashed border-[#BC8A5F] text-[#BC8A5F]'
            }`}
          >
            {isAnswerSubmitted
              ? isCorrect
                ? currentQ.answer
                : typedAnswer || '______'
              : typedAnswer || '______'}
          </span>
          <span>{parts[1]}</span>
        </div>

        {/* Listen Full Sentence Button */}
        <button
          onClick={() => audioService.speak(fullSentenceWithAnswer, 0.88)}
          className="inline-flex items-center gap-1.5 text-xs font-bold text-[#4B5D44] bg-[#E8EFE6] hover:bg-[#D5E4D1] border border-[#C6D8C2] px-3.5 py-1.5 rounded-full transition-all cursor-pointer"
        >
          <Volume2 className="w-3.5 h-3.5" />
          <span>Nghe cả câu</span>
        </button>
      </div>

      {/* Word Suggestions Chips */}
      {!isAnswerSubmitted && (
        <div className="space-y-2">
          <p className="text-xs font-black uppercase tracking-wider text-[#5C6B57] px-1">
            Gợi ý từ vựng (chọn hoặc gõ vào ô):
          </p>
          <div className="flex flex-wrap gap-2">
            {wordBankOptions.map((w, idx) => (
              <button
                key={idx}
                type="button"
                onClick={() => handleSelectWordChip(w)}
                className={`px-3.5 py-2 rounded-xl border text-xs sm:text-sm font-bold transition-all cursor-pointer ${
                  typedAnswer.toLowerCase() === w.toLowerCase()
                    ? 'bg-[#4B5D44] text-white border-[#4B5D44] shadow-xs'
                    : 'bg-white hover:bg-[#FAF7F2] text-[#2D332A] border-[#E5DDD0]'
                }`}
              >
                {w}
              </button>
            ))}
          </div>
        </div>
      )}

      {/* Input Form */}
      <form onSubmit={handleSubmitAnswer} className="space-y-3">
        <div className="relative">
          <input
            id="sentence-blank-input"
            ref={inputRef}
            type="text"
            value={typedAnswer}
            onChange={(e) => setTypedAnswer(e.target.value)}
            disabled={isAnswerSubmitted}
            placeholder="Gõ từ còn thiếu vào đây..."
            autoComplete="off"
            autoCapitalize="off"
            spellCheck="false"
            className={`w-full px-5 py-4 rounded-2xl border text-base sm:text-lg font-bold transition-all outline-none ${
              isAnswerSubmitted
                ? isCorrect
                  ? 'bg-[#E8EFE6] border-[#4B5D44] text-[#384732] ring-2 ring-[#4B5D44]/30'
                  : 'bg-[#F9EBE9] border-[#E8C2BD] text-[#88372A]'
                : 'bg-white border-[#E5DDD0] text-[#2D332A] focus:border-[#4B5D44] focus:ring-4 focus:ring-[#4B5D44]/10 shadow-xs'
            }`}
          />
          {!isAnswerSubmitted && (
            <button
              type="submit"
              disabled={!typedAnswer.trim()}
              className="absolute right-2.5 top-2.5 bottom-2.5 px-5 rounded-xl bg-[#4B5D44] hover:bg-[#3D4C37] disabled:bg-[#D5DDD0] text-white font-extrabold text-xs transition-all flex items-center gap-1.5 cursor-pointer disabled:cursor-not-allowed"
            >
              <span>KIỂM TRA</span>
              <ArrowRight className="w-3.5 h-3.5" />
            </button>
          )}
        </div>
      </form>

      {/* Feedback & Navigation Bar */}
      {isAnswerSubmitted && (
        <div className="bg-white rounded-3xl p-4 sm:p-5 border border-[#E5DDD0] shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-3 animate-fadeIn">
          <div className="flex items-center gap-2.5">
            {isCorrect ? (
              <div className="w-8 h-8 rounded-full bg-[#E8EFE6] text-[#4B5D44] flex items-center justify-center shrink-0">
                <Check className="w-5 h-5" />
              </div>
            ) : (
              <div className="w-8 h-8 rounded-full bg-[#F9EBE9] text-[#88372A] flex items-center justify-center shrink-0">
                <X className="w-5 h-5" />
              </div>
            )}
            <div>
              <p className={`font-black text-sm ${isCorrect ? 'text-[#4B5D44]' : 'text-[#88372A]'}`}>
                {isCorrect ? 'Chính xác! (+10 điểm)' : 'Chưa chính xác!'}
              </p>
              <div className="flex items-center gap-2 text-xs text-[#5C6B57]">
                <span>Đáp án đúng:</span>
                <span className="font-bold text-[#2D332A] text-sm font-serif">{currentQ.answer}</span>
                {matchedVocabItem && <span>• {matchedVocabItem.meaning_vi}</span>}
              </div>
            </div>
          </div>

          <div className="flex items-center gap-2 shrink-0">
            {!isCorrect && (
              <button
                onClick={handleRetryCurrent}
                className="px-4 py-2.5 rounded-xl bg-[#FAF7F2] hover:bg-[#F2ECE1] text-[#2D332A] border border-[#E5DDD0] font-extrabold text-xs transition-all flex items-center gap-1.5 cursor-pointer"
              >
                <RotateCcw className="w-3.5 h-3.5" />
                <span>Thử lại (5đ)</span>
              </button>
            )}

            <button
              onClick={handleNextQuestion}
              className="px-5 py-2.5 rounded-xl bg-[#4B5D44] hover:bg-[#3D4C37] text-white font-extrabold text-xs shadow-xs transition-all flex items-center gap-1.5 cursor-pointer ml-auto sm:ml-0"
            >
              <span>{currentIndex < questions.length - 1 ? 'CÂU TIẾP THEO' : 'XEM KẾT QUẢ'}</span>
              <ArrowRight className="w-3.5 h-3.5" />
            </button>
          </div>
        </div>
      )}
    </div>
  );
};
